...
-- question 3--
select count(distinct customer.cus_id) as customerCount, customer.cus_gender from
customer 
inner join 
...
select count(*) as "total customers", cus_gender from customer
where customer.cus_id  in (select distinct (cust_id) from `order` where ord_amount >=3000 )
group by cus_gender;

-- question 4--
select  product.prod_name,'order'.*
from 'order', supplier_pricing, product
where 'order'. cus_id=2
and 'order'.pricing_id= supplier_pricing.pricing_id
and supplier_pricing.pro_id=product.prod_id;

-- question 5--

select supplier.* from supplier
where supplier.supp_id in (select supp_id from supplier_pricing group by supp_id)
having count(supp_id >1);

-- question 6--

select 'category'.cat_id, 'category'.cat_name, 'supplier_pricing'.supp.price, min (t.min_price) as min_prices
from category
inner join
(select product.cat_id, product.pro_name,t2* from product) 
inner join
(select pro_id, min(supp_price) as min_prices from supplier_pricing group by prod_id)
...
join 'order' on supplier_pricing.pricing_id= 'order'.pricing.id
where 'order'. ord_date > '2021-10-05';

select pr.pro_id as ' id', pr.pro_name as 'name of the product' from product pr, supplier_pricin sp, 'order'o
